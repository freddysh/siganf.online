...
export default {
    ...
    state:{
        ...
        endpoint1:'https://api.siganf.online' || null
    },
    actions:{
        ...
        async login({commit},credenciales){
            try {
                ...
                let rpt1 = await axios.get(`https://api.siganf.online/api/v1/login/${credenciales.txt_user.trim()}/${credenciales.txt_pw.trim()}`);
                if (rpt1.data.length) {
